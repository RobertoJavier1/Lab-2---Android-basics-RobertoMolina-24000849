...
@Composable
//full screen is the principal state
...
//     var diceValue by rememberSaveable { mutableIntStateOf(MIN_DICE_VALUE) }
    ...
    //total value, the sum of all de stats
...
//    var isRolling by remember { mutableStateOf(false) }
    var rollingStat by remember {mutableStateOf<String?>(null)}//to identify which stat is rolling it can be null
...
//    }
    ...
    fun rollStat(nameStat: String, setValue: (Int) -> Unit ){
        if(rollingStat != null) return // to avoid 2 animation at the same time

        coroutineScope.launch {
            rollingStat = nameStat
            repeat(ANIMATION_ITERATIONS){
                setValue((MIN_DICE_VALUE..MAX_DICE_VALUE).random())
                delay(ANIMATION_DELAY_MS)
            }
...
//            }
//
            // to separate elements of the column
            // dp = density-independent pixels it adapts to big and small screens
            Spacer(modifier = Modifier.height(24.dp))
//
            //final stats
            Text(
                text = "Total: $totalstat",
                fontSize = 24.sp,
                style = MaterialTheme.typography.titleLarge,
                fontWeight = FontWeight.Bold,
...
fun StatRow(name: String, value: Int,isRolling: Boolean, onRoll: () -> Unit){ //Unit is equivalent to void on java
    Card(
        modifier = Modifier
            .fillMaxWidth()
            .padding(vertical = 8.dp), //sepation between cards
        shape = MaterialTheme.shapes.large //rounded edges
    ) {
        Row(
            modifier = Modifier
                .fillMaxWidth() //fill all the screen
                .padding(horizontal = 16.dp, vertical = 12.dp), //internal padding
            verticalAlignment = Alignment.CenterVertically,
            horizontalArrangement = Arrangement.SpaceBetween
            //the first one is placed on the left and the last one is placed on the right
...
@Preview(
    showBackground = true,
    showSystemUi = true,
    name = "RPG Character Sheet Preview"
)
@Composable
